...
{
    internal class TensorflowWorker
    {
        private const string ONNX_MODEL_PATH = @".\common_old.onnx";
        public static void Predict(Image<Argb32> img)
        {
            img.Mutate(img_ => img_
            .Grayscale()
            .Resize(img.Width / (img.Height / 64), 64, false)
            );
        ...
        public static void Fix(Image<Argb32> img)
        {
            Tensor<float> input = new DenseTensor<float>(new[] { 1, 1, 64, img.Width });
            ...
            var result = (DenseTensor<long>)resultsArray[0].Value;
            result.ToList().ForEach(x =>
            {
                var chars = charset[(int)x];
                if (chars != "")
                {
                    Console.Write(chars);
                }
        ...
        {
            "", "掀", "袜", "顧", "徕", "榱", "荪", "浡", "其", "炎", "玉", "恩", "劣", "徽", "廉", "桂", "拂",
                                  "鳊", "撤",
